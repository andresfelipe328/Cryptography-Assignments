...
def Dowload(NEW_SOCKET, portNumber, scheme, upFile):
    data = ""
    plaintext = ""
    final = ""
    delete = False
        ...
        if portNumber == (BASE_PORT + 3):
            ...
            while fileBuffer:
                ...
                if fileBuffer[len(fileBuffer) - 4:] == "stop".encode(FORMAT):
                    ...
                    fileBuffer = fileBuffer[0:len(fileBuffer) - 4]
                    newFile.write(fileBuffer)
                    break
                newFile.write(fileBuffer)
...
def TransferFile(conn, address, data):
    ...
    with open(path, 'rb') as outFile:
        ...
        if address[1] == (BASE_PORT + 8) and scheme == "rsa":
            with open('PublicKeys/publicTwo.pem', 'rb') as filePublicKey:
                keyData = filePublicKey.read()
                publicKey = rsa.PublicKey.load_pkcs1_openssl_pem(keyData)
            filePublicKey.close()
...
def CommunicationWithClients(conn, address):
    while True:
        ...
        if not data or data == "quit":
            break
        TransferFile(conn, address, data)
    conn.close()
